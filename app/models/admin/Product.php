...
//модель продуктів для створення,редагування адміном
class Product extends AppModel {
    ...
    public function editRelatedProduct($id, $data){
        ...
        // якщо змінилися пов'язані товари - видалимо та запишемо нові
    ...
    public function editFilter($id, $data){
        ...
        // якщо менеджер прибрав фільтри - видаляємо їх
        if(empty($data['attrs']) && !empty($filter)){
            \R::exec("DELETE FROM attribute_product WHERE product_id = ?", [$id]);
            return;
        }
        // якщо фільтри додаються
        ...
        // якщо змінилися фільтри - видалимо та запишемо нові
        if(!empty($data['attrs'])){
    ...
    public function uploadImg($name, $wmax, $hmax){
        ...
        $types = array("image/gif", "image/png", "image/jpeg", "image/pjpeg", "image/x-png"); // масив допустимих розширень
        if($_FILES[$name]['size'] > 2097152){
            $res = array("error" => "Помилка! Максимальна вага файлу - 2 Мб!");
        ...
        if(!in_array($_FILES[$name]['type'], $types)){
            $res = array("error" => "Допустимі розширення - .gif, .jpg, .png");
            exit(json_encode($res));
        }
    ...
    public static function resize($target, $dest, $wmax, $hmax, $ext){
        ...
        if($ext == "png"){
            imagesavealpha($newImg, true); // альфа канал
            $transPng = imagecolorallocatealpha($newImg,0,0,0,127); // додаємо прозорості
            imagefill($newImg, 0, 0, $transPng); // заливка
        }

        imagecopyresampled($newImg, $img, 0, 0, 0, 0, $wmax, $hmax, $w_orig, $h_orig); // копіюємо та ресайзимо фото
        switch($ext){
            case("gif"):
